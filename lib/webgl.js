...
imageDataCanvas.width = 4096;
imageDataCanvas.height = 4096;
const imageDataCtx = imageDataCanvas.getContext('2d');
function _getImageData(o) {
  if (o.buffer) { // typed array
    return o;
  } else if (o.data) { // ImageData
    return o.data;
  } else if (o.tagName || o instanceof ImageBitmap) { // drawable
    const width = o.naturalWidth || o.videoWidth ||  o.width;
    const height = o.naturalHeight || o.videoHeight || o.height;
    imageDataCtx.clearRect(0, 0, width, height);
    imageDataCtx.drawImage(o, 0, 0);
    const data = imageDataCtx.getImageData(0, 0, width, height).data;
    console.log('got data', o, data, new Error().stack);
    return data;
  } else { // unknown
    console.warn('unknown image format:', o);
...
var _drawArraysInstancedANGLE = gl.drawArraysInstancedANGLE;
gl.drawArraysInstancedANGLE = function drawArraysInstancedANGLE(mode, first, count, primcount) {
  if (!(arguments.length === 3 && typeof mode === "number" && typeof first === "number" && typeof count === "number" && typeof primcount === "number")) {
    throw new TypeError('Expected drawArraysInstancedANGLE(number mode, number first, number count, number primcount)');
  }
...
var _drawElementsInstancedANGLE = gl.drawElementsInstancedANGLE;
gl.drawElementsInstancedANGLE = function drawElementsInstancedANGLE(mode, count, type, offset) {
  if (!(arguments.length === 4 && typeof mode === "number" && typeof count === "number" && typeof type === "number" && typeof offset === "number" && typeof primcount === "number")) {
    throw new TypeError('Expected drawElementsInstancedANGLE(number mode, number count, number type, number offset, number primcount)');
  }
  return _drawElementsInstancedANGLE(mode, count, type, offset, primcount);
}
...
gl.texImage2D = function texImage2D(target, level, internalformat, width, height, border, format, type, pixels) {
  ...
  if (arguments.length == 6) {
    ...
    if(!(
        ...
        (border==null || (typeof border === "object" && typeof border.width === 'number' && typeof border.height === 'number')))) {
    ...
    type=height;
    format=width;
    width = border ? border.width : 1;
    height = border ? border.height : 1;
...
var _vertexAttribDivisorANGLE = gl.vertexAttribDivisorANGLE;
gl.vertexAttribDivisorANGLE = function vertexAttribDivisorANGLE(index, divisor) {
  if (!(arguments.length === 2 && typeof index === "number" && typeof divisor === "number")) {
    throw new TypeError('Expected vertexAttrib4fv(number index, number divisor)');
  }
