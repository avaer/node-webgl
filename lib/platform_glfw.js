...
module.exports = function () {
  ...
  platform = {
    ...
    setCursor: function(enabled) {
      GLFW.SetInputMode(window, GLFW.CURSOR, enabled ? GLFW.CURSOR_NORMAL : GLFW.CURSOR_DISABLED);
    },
        ...
        if (name === 'resize') {
          ...
          callback = evt => {
            ...
            if (this.pointerLockElement) {
              evt.movementX = evt.pageX - (this.style.width / 2);
              evt.movementY = evt.pageY - (this.style.height / 2);
            }
            tmpcb.call(self,evt);

            if (this.pointerLockElement) {
              this.setCursorPos(this.style.width / 2, this.style.height / 2);
            }
          };
